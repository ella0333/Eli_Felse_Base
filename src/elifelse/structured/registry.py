...
CORE_SCHEMAS: dict[str, Schema] = {
    # Menus are lettered: the enum is the letters, and the caller maps the
    # letter back to its option (see loop/menus.py).
    "bedtime_menu": menu_schema(["A", "B"]),
    "chat_response": {
        "type": "object",
        "properties": {
            "thinking": {"type": "string"},
    ...
    "nap_interrupted": menu_schema(["A", "B"]),
